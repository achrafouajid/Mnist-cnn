...
If mnist_cnn_model.h5 doesn't exist, the script trains a CNN with 5-fold CV,
saves the final model, then starts the Flask server with a /predict_digit POST endpoint.
"""
...
MODEL = None  # global model loaded at startup
K_FOLDS = 5
...
def train_and_save_model(model_path: str = MODEL_PATH, epochs: int = 10, k_folds: int = K_FOLDS) -> None:
    """
    Train a simple CNN on MNIST using K-Fold Cross Validation.
    The final model is retrained on all data and saved to model_path.
    """
